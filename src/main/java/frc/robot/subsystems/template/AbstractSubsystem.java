...
public abstract class AbstractSubsystem extends SubsystemBase {
    ...
    public AbstractSubsystem(SubsystemType type, int motorID, TrapezoidProfile.Constraints constraints,
    ...
    // Configurations
    public void configureMotor(boolean isInverted, boolean isBrakeMode, double supplyCurrentLimit,
            double statorCurrentLimit) {
        motorConfig.MotorOutput.Inverted = isInverted ? InvertedValue.Clockwise_Positive
    ...
    public void setVelocity(double rps) {
        if (type == SubsystemType.LINEAR || type == SubsystemType.PIVOT)
            return;

        this.goal = rps;
        followLastMechProfile = false;
        motor.setNeutralMode(NeutralModeValue.Coast);
        if (type == SubsystemType.ROLLER)
    ...
    public void setPosition(double goal) {
        if (type == SubsystemType.FLYWHEEL)
            return;

        if (type == SubsystemType.LINEAR && goal < mechMin)
            goal = mechMin;
        else if (type == SubsystemType.LINEAR && goal > mechMax)
            goal = mechMax;
        ...
        switch (type) {
            ...
            default -> {
                if (isVelocity)
                    ...
                    return getMechRot() >= goal - lowerTolerance
    ...
    public void periodic() {
        if (followLastMechProfile)
            followLastMechProfile();
        if (type == SubsystemType.ROLLER && getMotorVelocity() == 0 && getMotorRot() != 0) {
